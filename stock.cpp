...
void addStockData(pqxx::work& W, const std::string& symbol, const std::string& timestamp, double open, double high, double low, double close, int volume) {
    std::string sql = "INSERT INTO Stocks (symbol, timestamp, open, high, low, close, volume) VALUES (" 
        + W.quote(symbol) + ", " 
        + W.quote(timestamp) + ", " 
        + W.quote(open) + ", " 
        + W.quote(high) + ", " 
    ...
    W.exec(sql);
    W.commit();
    std::cout << "Stock data added successfully for symbol: " << symbol << " at " << timestamp << "\n";
}
